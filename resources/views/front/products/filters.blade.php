            ...
            @foreach ($filters as $filterKey => $filter)
                ...
                <ul>
                    @foreach ($filter as $category)
                       <li class=""><a href="{{ url('/'.$category['url']) }}">{{$category['category_name']}}</a></li> 
                       @if (count($category['sub_categories']) > 0)
                           <ul>
                            @foreach ($category['sub_categories'] as $sub_category)
                                <li class=""><a href="{{ url('/'.$sub_category['url']) }}">{{$sub_category['category_name']}}</a></li>
                            @endforeach
                           </ul>
                       @endif
                    @endforeach
                ...
                <form class="facet-form" action="#" method="post">
                    <div class="associate-wrapper">
                        ...
                        <div>
                            <input type="checkbox" class="check-box size" id="{{$filterKey}}{{ $key }}" name="{{$filterKey}}[]" value="{{ $childFilters }}">
